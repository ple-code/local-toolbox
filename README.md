...
当前第一个工具是 `html2pdf`：打开网页，把页面导出为 PDF；多个 URL 会打包成 ZIP 下载。
...
- 点击“开始导出”会触发浏览器下载，并把任务、任务日志和生成文件保存到本地 SQLite/文件目录。
- 一个 URL 下载 PDF，多个 URL 下载 ZIP。
- 历史任务里可以查看任务详情、下载已生成文件；失败任务可以回填到新建任务里重试，进行中任务可以手动停止。
- 同一时间只执行一个导出任务；并发提交的任务会进入“排队中”，等待前一个任务结束后继续。
...
- 知识星球使用微信扫码登录：任务开始后会打开登录页、勾选协议并显示二维码截图，扫码后点击“我已扫码，继续导出”。
