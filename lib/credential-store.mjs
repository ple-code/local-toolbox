...
export class CredentialStore {
  ...
  async save(service, { username, password, loginUrl }) {
    await this.init();
  ...
  async getMeta(service) {
    ...
    return {
      ...
      service: row.service,
      username: row.username,
      loginUrl: decryptJson(row.encrypted_payload, this.secret).loginUrl || "",
      updatedAt: row.updated_at
    };
  }

  async saveLoginUrl(service, loginUrl) {
    await this.init();
    const cleanService = String(service || "").trim();
    const cleanLoginUrl = String(loginUrl || "").trim();
    if (!cleanService) {
      throw new Error("service is required.");
    }
    if (!cleanLoginUrl) {
    ...
    this.db.prepare(`
      INSERT INTO platform_settings (service, login_url, updated_at)
      VALUES (?, ?, datetime('now'))
      ON CONFLICT(service) DO UPDATE SET
        login_url = excluded.login_url,
        updated_at = excluded.updated_at
    `).run(cleanService, cleanLoginUrl);
  }

  async getLoginUrlMeta(service) {
    await this.init();
    const row = this.db.prepare(`
      SELECT service, login_url, updated_at
