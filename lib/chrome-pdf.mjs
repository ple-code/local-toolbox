...
async function originalScrollScreenshotPdf(cdp, sessionId) {
  ...
  for (const requestedTop of positions) {
    ...
    const remainingHeight = Math.max(0, initial.totalHeight - Math.max(capturedBottom, metrics.contentTop));
    const clipHeight = Math.min(Math.max(0, metrics.clip.height - cropTop), remainingHeight || metrics.clip.height);
    if (clipHeight < 20) {
    ...
    capturedBottom = Math.max(capturedBottom, metrics.contentTop + cropTop + clipHeight);
    previousTop = metrics.actualTop;
  }
