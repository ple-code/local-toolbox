...
export async function prepareBrowserPrintPageForPdf(cdp, sessionId, { platform = "generic" } = {}) {
  const result = await cdp.send("Runtime.evaluate", {
    ...
    expression: `new Promise(async (resolve) => {
      ...
      document.body.removeAttribute('style');
      document.body.replaceChildren(root);
      document.title = document.title || clone.querySelector('#activity-name, h1')?.textContent?.trim() || '';
      const style = document.createElement('style');
...
export class ChromePdfSession {
  ...
  async pdfForUrl(url, { waitMs = this.waitMs, closeTab = true, platform = "generic" } = {}) {
    ...
    try {
      ...
      let pdf;
      if (!browserPrintPlatforms.has(platform) && pageInfo.printRoot?.enabled) {
