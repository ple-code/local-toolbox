...
export class ChromePdfSession {
  ...
  async probeAuthState(url, { platform = "generic", waitMs = this.waitMs } = {}) {
    this.log("探测当前浏览器登录态", { platform, url });
    const tab = await this.open(url, { waitMs, waitForLoad: false });
    try {
      const state = await tab.cdp.send("Runtime.evaluate", {
        awaitPromise: true,
        returnByValue: true,
        expression: `(async () => {
          const platform = ${JSON.stringify(platform)};
          ...
          for (let attempt = 0; attempt < 16; attempt += 1) {
            ...
            const contentText = compactText(
              document.querySelector('#article-content-container, article, main, .article, .article-content, .markdown-body, [class*="article"], [class*="content"]')?.innerText || ''
            );
            if (Math.max(bodyText.length, contentText.length) >= 1500 || /用户登录|账号登录|验证码登录|密码登录|微信扫码|二维码/.test(bodyText)) {
              break;
            }
            await sleep(500);
          }
          ...
          const article = document.querySelector('#article-content-container, article, main, .article, .article-content, .markdown-body, [class*="article"], [class*="content"]');
          const articleText = compactText(article?.innerText || '');
          const loginTextPattern = /登录|请登录|登录后|扫码|二维码|验证码|安全验证|手机号|密码登录|账号登录/;
          let authenticated = true;
          let reason = 'no-auth-required';
          if (platform === 'geektime') {
            const onAccountLogin = host === 'account.geekbang.org' && /\\/log(in)?|\\/signin/.test(location.pathname);
            const hasArticleBody = Math.max(text.length, articleText.length) >= 1800 &&
              !/登录后可阅读|登录后继续|请先登录|开通会员后继续|购买后阅读/.test(text);
            const hasLoginShell = onAccountLogin || (
              loginTextPattern.test(text) &&
              (/用户登录|账号登录|密码登录|验证码登录|登录后/.test(text) || text.length < 1200)
            );
            authenticated = hasArticleBody && !hasLoginShell;
            reason = authenticated ? 'article-content-visible' : (onAccountLogin ? 'on-login-page' : 'article-content-not-visible');
          } else if (platform === 'zsxq') {
            const onLogin = /\\/login\\/?$/.test(location.pathname) || /login|signin|passport/i.test(currentUrl);
            const hasArticleBody = text.length >= 1800 && !/微信扫码|微信扫一扫|请使用微信|授权|验证码/.test(text);
            authenticated = hasArticleBody && !onLogin;
            reason = authenticated ? 'article-content-visible' : (onLogin ? 'on-login-page' : 'article-content-not-visible');
          }
          return {
            authenticated,
            reason,
            ...
            needsManualAction: /验证码|滑块|扫码|二维码|安全验证|人机/.test(text)
          };
        })()`
      }, tab.sessionId, 15000);
      if (state.exceptionDetails) {
        throw new Error(state.exceptionDetails.text || state.exceptionDetails.exception?.description || "登录态探测脚本执行失败");
      }
      const value = state.result.value || {};
      this.log("登录态探测完成", value);
      return value;
    } finally {
  ...
  async pdfForUrl(url, { waitMs = this.waitMs, closeTab = true, platform = "generic", returnPageInfo = false } = {}) {
    ...
    try {
      ...
      let pdf;
      if (platform === "wechat" || platform === "zsxq") {
        this.log(`${platform === "wechat" ? "微信公众号" : "知识星球"}使用截图分页生成 PDF`, {
          url,
