...
function buildPdfFromJpegs(pages) {
  ...
  pages.forEach((page, index) => {
    ...
    const pageHeightPt = pageWidthPt * page.height / page.width;
    const content = `q ${numberForPdf(pageWidthPt)} 0 0 ${numberForPdf(pageHeightPt)} 0 0 cm /Im${index + 1} Do Q\n`;
...
async function originalScrollScreenshotPdf(cdp, sessionId) {
  ...
  const pages = [];
  const positions = [];
  for (let top = 0; top < maxScrollTop; top += visibleHeight) {
    positions.push(top);
  }
  ...
  for (const requestedTop of positions) {
    const metricsResult = await cdp.send("Runtime.evaluate", {
      ...
      expression: `new Promise(async (resolve) => {
        ...
        if (actualTop === previousTop) {
          resolve({ ok: false, reason: 'duplicate scroll position', actualTop });
          return;
        }
        const articleRect = article.getBoundingClientRect();
        const rootRect = scrollRoot.getBoundingClientRect();
        ...
        resolve({
          ok: visibleBottom > visibleTop + 20,
          actualTop,
          clip: {
            x: Math.max(0, Math.round(articleRect.left)),
    ...
    const isLast = metrics.actualTop >= maxScrollTop - 4;
    const overlappedByPrevious = Math.max(0, metrics.actualTop - previousTop);
    const clipHeight = isLast && previousTop >= 0
      ? Math.min(metrics.clip.height, Math.max(180, initial.scrollHeight - metrics.actualTop))
      : Math.min(metrics.clip.height, previousTop >= 0 && overlappedByPrevious > 0 ? overlappedByPrevious : metrics.clip.height);
...
export async function screenshotPdf(cdp, sessionId) {
  ...
  const metricsResult = await cdp.send("Runtime.evaluate", {
    returnByValue: true,
    expression: `(() => {
      const root = document.querySelector('#html2pdf-print-root') || document.body || document.documentElement;
  ...
  async pdfForUrl(url, { waitMs = this.waitMs, closeTab = true } = {}) {
    ...
    try {
      ...
      if (pageInfo.printRoot?.enabled) {
        ...
        pdf = await printToPdf(tab.cdp, tab.sessionId);
      }
      if (pdf.length < 5000) {
