  ...
  async acceptLoginAgreement() {
    ...
    const result = await this.loginTarget.cdp.send("Runtime.evaluate", {
      ...
      expression: `new Promise(async (resolve) => {
        ...
        if (!clicked) {
          const candidates = Array.from(document.querySelectorAll('label, span, div, i'))
            .filter(visible)
            .filter((node) => /我已阅读|用户协议|隐私政策|同意/.test(textOf(node)) || /checkbox|agree/i.test(String(node.className || '')));
          for (const node of candidates.slice(0, 4)) {
            click(node);
            clicked = true;
            await sleep(250);
            break;
    ...
    this.log("登录协议处理完成", result.result.value || {});
    return result.result.value;
  }
