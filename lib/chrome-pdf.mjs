...
export class ChromePdfSession {
  ...
  async open(url, { waitMs = this.waitMs, waitForLoad = true } = {}) {
    ...
    if (!target.url || target.url === "about:blank") {
      this.log("标签页为空白，跳转到目标地址", { url });
      await pageCdp.send("Runtime.evaluate", {
        expression: `location.href = ${JSON.stringify(url)}`
      }, undefined, 5000).catch(() => undefined);
    }
    const ready = await waitForDocumentReady(pageCdp, undefined, waitForLoad ? 30000 : Math.max(8000, waitMs + 5000), url);
    this.log("页面已准备就绪", {
      url: ready?.url || url,
      title: ready?.title || ""
    });
  ...
  async autoLogin({ username, password, loginUrl = DEFAULT_LOGIN_URL, waitMs = this.waitMs }) {
    ...
    this.log("自动登录结果", {
      currentUrl: loginResult.result?.value?.currentUrl || "",
      title: loginResult.result?.value?.title || "",
      submitted: loginResult.result?.value?.submitted,
      needsManualAction: loginResult.result?.value?.needsManualAction,
      reason: loginResult.result?.value?.reason
    });
