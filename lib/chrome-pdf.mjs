...
export class ChromePdfSession {
  ...
  async open(url, { waitMs = this.waitMs } = {}) {
    await this.start();
    const { targetId } = await this.cdp.send("Target.createTarget", { url: "about:blank" });
    const { sessionId } = await this.cdp.send("Target.attachToTarget", { targetId, flatten: true });
    await navigateAndWait(this.cdp, sessionId, url, waitMs);
    await this.closeOtherBlankTabs(targetId);
    return { targetId, sessionId };
  }

  async openForLogin(url, options) {
    await this.closeLoginTab();
    this.loginTarget = await this.open(url, options);
    return { profileDir: this.profileDir };
  }
  ...
  async autoLogin({ username, password, loginUrl = DEFAULT_LOGIN_URL, waitMs = this.waitMs }) {
    ...
    await sleep(1200);
    const loginResult = await this.cdp.send("Runtime.evaluate", {
      awaitPromise: true,
      returnByValue: true,
      expression: `new Promise(async (resolve) => {
  ...
  async getLoginState() {
    if (!this.loginTarget?.sessionId || !this.cdp) {
      return { open: false };
    }
    const state = await this.cdp.send("Runtime.evaluate", {
      returnByValue: true,
      expression: `(() => {
  ...
  async captureLoginScreenshot() {
    ...
    await this.cdp.send("Page.bringToFront", {}, this.loginTarget.sessionId).catch(() => undefined);
  ...
  async closeOtherBlankTabs(keepTargetId) {
    ...
    const { targetInfos } = await this.cdp.send("Target.getTargets").catch(() => ({ targetInfos: [] }));
  ...
  async pdfForUrl(url, { waitMs = this.waitMs, closeTab = true } = {}) {
    ...
    try {
      await preparePageForPdf(this.cdp, tab.sessionId);
      return await printToPdf(this.cdp, tab.sessionId);
    } finally {
      if (closeTab) {
