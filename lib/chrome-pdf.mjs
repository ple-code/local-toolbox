  ...
  for (let pageIndex = 0; capturedBottom < initial.totalHeight - 8 && pageIndex < 240; pageIndex += 1) {
    ...
    const metricsResult = await cdp.send("Runtime.evaluate", {
      ...
      expression: `new Promise(async (resolve) => {
        ...
        const breakpoints = Array.from(article.querySelectorAll('p,h1,h2,h3,h4,h5,h6,li,pre,blockquote,table,img,figure,section'))
          .map((node) => {
            const rect = node.getBoundingClientRect();
            const style = getComputedStyle(node);
            ...
            return Math.round(rect.bottom - articleRect.top);
          })
